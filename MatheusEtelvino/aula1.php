<html>
    <head>
        <title>Primeiro Programa PHP</title>
</head>
<body>
    <?php
        echo "<h1>Hellow World, PHP-7</h1>";
    ?>

    <?php
        phpinfo();
    ?>

    <?php 
        print "teste <br>";
        print "Olá, Mundo <br>";
        print "Escape 'chars' são os MESMOS como em C\n <br>";
        print "Você pode ter quebras de linhas em uma string. <br>";
        print 'Uma string pode usar "aspas-dupals". Isso é muito legal! <br>';
        print 'Ainda pode-se usar aspas simples dessa forma It\'s cool !<br>';
    
    ?>

    <?php 
        echo "<h2 align='center'>
        O mei programa está ecoando corretamente no meu servidor PHP!
        </h2>";
    ?>
<body>

</html>